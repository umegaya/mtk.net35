...
namespace SimpleMigrations
{
    ...
    public class AssemblyMigrationProvider : IMigrationProvider
    {
        ...
        public MigrationAttribute GetMigrationAttribute(Type type) {
            foreach (var t in type.GetCustomAttributes(true)) {
                if (t is MigrationAttribute) {
                    return t as MigrationAttribute;
                }
            }
            return null;
        }
        ...
        public IEnumerable<MigrationData> LoadMigrations()
        {
            var migrations = from type in this.migrationAssembly.GetTypes()
                             let attribute = GetMigrationAttribute(type)
